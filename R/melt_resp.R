...
melt_resp <- function(input, info.data, O2.unit) {
  ...
  pre_output <- data.table::data.table(Date.Time = rep(input$Date.Time, each = sum(grepl("Ox", colnames(input)))),
                       Chamber.No = rep(paste0("CH", c(1:sum(grepl("Phase", colnames(input))))), length.out = nrow(input) * sum(grepl("Phase", colnames(input)))),
  ...
  if (!missing(info.data)) {
    ...
    link <- match(pre_output$Chamber.No, info.data$Chamber.No)
    ...
    by.chamber <- split(pre_output, pre_output$Chamber.No)
